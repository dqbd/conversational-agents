...
export function getPrefixedObjects(msg: string | undefined) {
  const segments =
    msg
      ?.split("\n")
      .filter((i) => i.trim().startsWith("agent-"))
      .map((i) => i.split(/[=:]/).map((i) => i.trim()))
      .reduce<Record<string, unknown>>((memo, [key, value]) => {
        let objKey = key?.startsWith("agent-")
          ? key.substring("agent-".length)
          : key!

        objKey = objKey.startsWith("m_")
          ? objKey.substring("m_".length)
          : objKey

        objKey = objKey.toLowerCase()
        memo[objKey] = value?.replaceAll(/[#].*/g, "").trim() ?? ""
...
export function getTextContent(msg: string | undefined) {
  const prefix = "DAN:"
  return msg
    ?.split("\n")
    .filter((i) => i.trim().startsWith("DAN:"))
    .map((i) => i.substring(prefix.length).trim())
    .join("\n")
}
